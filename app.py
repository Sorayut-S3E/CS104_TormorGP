...
@contextmanager
def db_write():
    """commit อัตโนมัติ หรือ rollback ถ้าเกิด error"""
    conn = get_db()
    try:
        yield conn
        conn.commit()
...
def init_db():
    ...
    # Migration: เพิ่ม columns ที่อาจยังไม่มีใน DB เก่า
    existing = [row[1] for row in c.execute("PRAGMA table_info(merchandise)").fetchall()]
    for col in ('image_url', 'coat_img', 'shirt_img', 'helmet_img', 'cap_img'):
        if col not in existing:
            c.execute(f"ALTER TABLE merchandise ADD COLUMN {col} TEXT")

    # Seed teams
    if c.execute('SELECT COUNT(*) FROM teams').fetchone()[0] == 0:
        c.executemany("INSERT INTO teams(id,team,rider,owner,sponsor,highlight,standing) VALUES(?,?,?,?,?,?,?)", [
    ...
    if c.execute('SELECT COUNT(*) FROM tracks').fetchone()[0] == 0:
        c.executemany("INSERT INTO tracks(id,track,highlight,country) VALUES(?,?,?,?)", [
            (1,  'Circuit de Barcelona-Catalunya',        'Fast corners, technical sector 2',              'Spain'),
            (2,  'Mugello Circuit',                       'Longest straight in MotoGP, Tuscan hills',      'Italy'),
            (3,  'Losail International Circuit',          'First night race in MotoGP history',            'Qatar'),
            (4,  'Jerez-Ángel Nieto Circuit',            'Technical and twisty, home of Spanish fans',    'Spain'),
            (5,  'Le Mans Circuit Bugatti',               'Unpredictable weather, historic venue',         'France'),
            (6,  'Assen TT Circuit',                      'Cathedral of Speed, oldest Grand Prix circuit', 'Netherlands'),
            (7,  'Sachsenring',                           'Unique anti-clockwise layout',                  'Germany'),
            (8,  'Silverstone Circuit',                   'High-speed complex, challenging weather',       'United Kingdom'),
            (9,  'Red Bull Ring',                         'Short lap, high-speed corners',                 'Austria'),
            (10, 'Misano World Circuit Marco Simoncelli', 'Named after the late champion',                 'Italy'),
        ])

    # Seed merchandise
    if c.execute('SELECT COUNT(*) FROM merchandise').fetchone()[0] == 0:
        c.executemany("INSERT INTO merchandise(id,team_id,coat,shirt,helmet,cap,image_url) VALUES(?,?,?,?,?,?,?)", [
            ...
            (7,  7,  269.99, 82.99, 559.99, 36.99, 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5d/Ducati_logo.svg/320px-Ducati_logo.svg.png'),
    ...
    if c.execute('SELECT COUNT(*) FROM calendar').fetchone()[0] == 0:
        c.executemany("INSERT INTO calendar(id,date,track_id,ticket,rider) VALUES(?,?,?,?,?)", [
    ...
    if c.execute('SELECT COUNT(*) FROM results').fetchone()[0] == 0:
        c.executemany("INSERT INTO results(id,calendar_id,result,standing,records) VALUES(?,?,?,?,?)", [
    ...
    if request.method == 'POST':
        ...
        with db_write() as conn:
            if new_standing:
                ...
                if old_standing and old_standing != new_standing:
                    # เคลียร์ slot เดิมก่อน
                    conn.execute('UPDATE teams SET standing = NULL WHERE id=?', (id,))

                    if new_standing > old_standing:
                        # ย้ายลงไป (เช่น 2→4): ดึงทีมระหว่าง old+1 ถึง new ขึ้นมา -1
                        affected = conn.execute(
                            'SELECT id, standing FROM teams WHERE standing > ? AND standing <= ? ORDER BY standing ASC',
                            (old_standing, new_standing)
                        ).fetchall()
                        for row in affected:
                            conn.execute('UPDATE teams SET standing=? WHERE id=?',
                                         (row['standing'] - 1, row['id']))
                    else:
                        # ย้ายขึ้นไป (เช่น 4→2): ดัน ทีมระหว่าง new ถึง old-1 ลงไป +1
                        affected = conn.execute(
                            'SELECT id, standing FROM teams WHERE standing >= ? AND standing < ? ORDER BY standing DESC',
...
def teams_delete(id):
    with db_write() as conn:
        ...
        if deleted and deleted['standing']:
            # ดึงทีมที่อยู่ต่ำกว่าขึ้นมา เรียง ASC เพื่อไม่ให้ชนกัน
            affected = conn.execute(
                'SELECT id, standing FROM teams WHERE standing > ? ORDER BY standing ASC',
                (deleted['standing'],)
            ).fetchall()
            for row in affected:
                conn.execute('UPDATE teams SET standing=? WHERE id=?',
...
def merchandise():
    ...
    data = conn.execute('''
        SELECT m.id, t.team, m.coat, m.shirt, m.helmet, m.cap, m.team_id,
               m.image_url, m.coat_img, m.shirt_img, m.helmet_img, m.cap_img
    ...
    if request.method == 'POST':
        with db_write() as conn:
            conn.execute(
                ...
                (request.form['date'], request.form['track_id'],
                 request.form['ticket'], request.form['rider'])
            )
    ...
    if request.method == 'POST':
        with db_write() as conn:
            conn.execute(
                ...
                (request.form['date'], request.form['track_id'],
                 request.form['ticket'], request.form['rider'], id)
            )
        return redirect(url_for('calendar'))
    conn   = get_db()
